        ...
        <g id="Artboard" transform="translate(-400.000000, -178.000000)">
          <g id="Group" transform="translate(400.000000, 178.000000)">
            ...
            @if (!empty($site_setting))
                <h2 class="brand-text text-primary ms-1">{{ $site_setting->company_name }}</h2>
            @else
                <h2 class="brand-text text-primary ms-1">شركة الفسيل للمقاولات</h2>
            @endif
        </a>
        <div class="misc-inner p-2 p-sm-3">
            <div class="w-100 text-center">
              @if (!empty($site_setting))
                  <h2 class="mb-3">{{ $site_setting->message_home_page }}</h2>
              @else
