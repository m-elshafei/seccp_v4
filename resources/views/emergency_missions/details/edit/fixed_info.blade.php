    ...
    <div class="d-flex justify-content-between flex-sm-row flex-column">
      <div class="card-information">
        ...
        <div class="d-flex align-items-center mb-50">
          <h6 class="mb-0"> {!! Form::label('work_order_number', __('models/emergencyMissions.fields.mission_number').':') !!}
            {{$emergencyMission->work_order_number ?? $emergencyMission->mission_number }}</h6> &nbsp;
            @if(isset($work_order_status[$emergencyMission->status]))
            <span class="badge rounded-pill {{$work_order_status[$emergencyMission->status]['class']}}">{{$work_order_status[$workOrder->status]['title']}}</span>
            @else
            {{$emergencyMission->status}}
            @endif
        </div>
        <span class="card-number">{!! Form::label('work_type_id', __('models/emergencyMissions.fields.mission_typeـid').':') !!}
            {{$emergencyMission->workType->full_name ?? 'مهمة'}}</span>
      </div>
      ...
      <div class="d-flex flex-column text-start text-lg-end">
        ...
        <span class="mt-2" data-bs-toggle="tooltip" title="{{$workOrder->received_date->toDateString() ?? ''}}"> تاريخ المهمه
